...
async function main() {
  ...
  const messages = [
      {
        role: "system",
        content: `You are smart assist. Respond to the user question 
                  and use tools if needed to answer the query.
      ...
      // {
      //   role: "user",
      //   content: "What is the current weather of Lahore?",
      // },
    ]
...
async function webSearch({ query }) {
  const response = await tavly.search(query, {maxResults:2});
